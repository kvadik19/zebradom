<?php
if ( ! class_exists( 'WP_Bootstrap_Footerwalker' ) ) {
	class WP_Bootstrap_Footerwalker extends Walker_Nav_Menu {
...
if ( ! class_exists( 'WP_Bootstrap_Sidebarwalker' ) ) {
	class WP_Bootstrap_Sidebarwalker extends Walker_Nav_Menu {
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			if ( isset( $args->item_spacing ) && 'discard' === $args->item_spacing ) {
				$t = '';
				$n = '';
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( isset( $args->has_children ) && $args->has_children && 0 === $depth && $args->depth > 1 ) {
				$atts['href']          = '#';
				$atts['class']         = 'dropdown-sidebar';
				$icon_class_string = 'icon-chevron-down';
				$atts['id']            = 'menu-item-dropdown-' . $item->ID;
			} else {
				$atts['href'] = ! empty( $item->url ) ? $item->url : '#';
				// Items in dropdowns use .dropdown-item instead of .nav-link.
